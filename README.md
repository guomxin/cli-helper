...
- OpenClaw 当前兼容基线为 `2026.7.1`，AgentBridge 插件为 `0.4.49`。
...
截至 2026-08-20，中央注册表包含 75 个业务能力：
...
| 照明实验室 | 26 | 静态资产、运行态、RTU/单灯告警、巡检、RTU 遥测、报告、RTU 告警受控写入 |
...
读取能力覆盖静态概览、灯杆与资产、当前运行状态、RTU 告警、单灯告警、巡检、RTU 巡测和
报告导出。旧“漏电”入口保留兼容，但真实漏电只从 RTU 告警或 RTU 巡测读取。写入能力聚焦
