...
- 语雀第三系统一期已实现公共区、知识库目录、全文检索和单文档读取。其滑块与短信验证
  由用户在可信交互浏览器卡中完成；搜索不返回摘要，明确选择后的正文按规则脱敏；当前
  只开放独立 `yuque:read` scope，部署后的真实只读验收尚未完成；
...
用户名密码、短信/TOTP 验证码和 MFA 推送可以通过多阶段字段型认证卡处理。对于滑块和必须在原登录页完成的短信流程，可使用数据盲的交互式登录卡：中心 Broker 独占短期浏览器线程，卡片只接收指定登录页的画面并转发用户的指针、键盘和滚动事件，浏览器控制令牌仅存在于同源卡片 JavaScript 中；模型和 MCP 结果都不可见。语雀一期已经采用该模式，但不自动破解滑块。二维码同机扫码、USB Key、ActiveX、客户端证书和电子签章仍转入受控桌面接管或明确标记为不支持。
