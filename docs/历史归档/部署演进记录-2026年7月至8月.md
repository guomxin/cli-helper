...
当前仍使用管理员签发的 Bearer 和内部 CA，因此“添加 MCP 地址并授权”尚未完全一键化。标准 OAuth 2.1、浏览器身份绑定和每用户独立 OS/容器安全主体仍是后续生产化工作。详细说明见 [远程 MCP 低安装接入](../平台能力/远程模型上下文协议接入.md)。
...
一套共享 Token 工具。完整配置见
[OpenClaw 多用户身份路由](../架构设计/智能体宿主多用户身份路由.md)。
...
- 具体命令、故障解释和安全边界见 [开发验证与发布流程](../部署运维/开发验证与发布流程.md)。
...
- 增加源码退役守卫、文档链接和历史归档守卫。当前文档入口收敛到
  `docs/文档导航.md`，受治理写动作以 `docs/架构设计/受控写入模型.md` 为准，旧桥设计和
  早期写动作草案移入 `docs/历史归档/`，仅保留历史证据；
...
- 详细证据见 [整体系统验收报告](../验收记录/系统整体验收-2026-08-04.md)。
...
- 新增独立运行手册
  [Agent Workspace 反向 SSH 隧道](../部署运维/工作台反向安全隧道.md)，固定记录
