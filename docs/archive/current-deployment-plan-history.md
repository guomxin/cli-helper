...
> 静态业务字段卡统一支持
> 对话已知值预填；出差和请假提交撤销已闭环，补签、劳动合同续签与知识产权申报已有专用接收处理能力。
...
| 中心受治理写能力 | 已实现出差、请假、补签、会议、普通协同、周报、效能数据、差旅费、劳动合同续签、知识产权申报审批和流程撤销等工作流能力；业务卡按用户已提供信息预填，再经过核对、实时 prepare、独立授权、单次 commit 与权威回读。出差和请假正式提交与撤销、普通协同、周报知会及会议创建已完成真实闭环 |
...
### 15.53 2026-08-04 整体系统验收与自然接续

- OpenClaw 插件升级为 `0.4.22`。用户明确说“刚才、最近、上一个”等相对指代时，中心端在
  同一用户和宿主范围内选择最近任务并记录 `latest_relative_reference`，不再要求日常用户
  记忆任务号；存在歧义且没有相对指代时仍要求澄清。
- OA 跟踪事项改为从权威跟踪页读取 affair ID，再与当前账号已发、已办结果精确关联，保持
  待办、已发、已办和跟踪四个集合的独立语义。辛国茂与李世玉真实读取均通过。
- Workspace 启动守卫增加 90 秒活动 run 宽限期；真实活动 run 在宽限期内只报告排队进度，
  缺失 run 仍按 15 秒快速恢复，超过宽限期的活动 run 才进入兜底恢复。
- 双用户自然接续实测通过：辛国茂网页到 Telegram、李世玉网页到微信均复用原 Task，只新增
  一个详情 Operation，没有重复读取待办列表，10 类身份一致性违规则全部为 0。
- 经用户可信确认，以辛国茂身份正式提交一条测试出差申请，OA 已发权威回读成功；随后执行
  独立撤销并通过已发消失和待发撤销态双重确认。提交与撤销各一次，授权均已消费，最终 Task
...
- 系统验收后的界面复盘发现，撤销作为原提交任务的后续 Operation 复用了同一 `taskId`，
  Workspace 因而正确地更新同一 DOM 卡片，但用户看到的效果是撤销卡覆盖了提交卡。
- OpenClaw 插件 `0.4.23` 将 `oa_workflow_revoke_prepare` 定义为独立用户任务入口：首次撤销
  准备会建立新的 Task，字段填写、授权和结果回读继续绑定该撤销 Task，原提交 Task 和卡片
  保持不变。新撤销 Task 仍通过冻结的 `affairId` 关联原 OA 流程，不削弱幂等和审计关系。
- 用户可以直接说“撤销刚提交的出差申请”。智能体从当前会话或 OA 已发列表选择唯一近期事项；
  只有存在多个合理候选时才询问可读标题或日期，不要求用户提供 `affairId`、`processId`、
  `taskId` 或精确技术标题。撤销属于破坏性动作，最终独立授权卡不取消。
...
### 15.56 2026-08-04 管理控制台与 Task Hub 治理面升级

- 管理 API 升级为 `v2`，新增只读 `/api/coordination`，以安全投影统一展示任务、客户端
  Endpoint、跨端 Continuation、通知 Outbox 和隔离完整性；不返回聊天正文、任务摘要、
  业务字段、卡片 URL、外部聊天主体、会话路由、端点密钥或完整事件载荷。
- 管理控制台新增“多端任务”页，按任务、端点、接续、投递和隔离五个标签切换；总览增加
  活动任务、等待用户、活动端点、待投递和隔离异常。会话页区分实际登录、用户活动、保活
  资格和最近保活，用户、会话、能力、操作、交互、运行与审计列表统一支持搜索和状态筛选。
- 浏览器隔离预览完成登录、总览、标签切换、投递搜索和响应式检查；桌面 `1440×900` 与手机
  `390×844` 均无文本或控件重叠，浏览器控制台错误为 0。测试数据未连接任何真实下游系统。
- 本地门禁通过 Python `490 passed, 3 skipped`、`compileall`、JavaScript 语法和差异格式检查；
  发布脚本复验通过 Python `487 passed, 3 skipped, 199 subtests`、Workspace `20/20` 和
  `pip check`。提交 `ebc0499` 对应 Linux Release `ebc0499c0813`，未重启 OpenClaw Gateway。
- 部署后 `https://10.10.50.213:8782/healthz` 返回 `200` 和 Release `ebc0499c0813`，线上
...
### 15.61 2026-08-11 知识产权申报审批专用适配

- 辛国茂待办中的“【知识产权】知识产权申报审批单-软件著作权”提供了真实契约样本。
  中央会话只读探针确认模板 `-2986710992990286032`、表单
  `-6972097064001584076`、`approve / 审批` 节点和 `agree / disagree` 态度；探针
  记录写控件点击、协同写请求和授权创建均为 0。
- 新增 `oa.intellectual_property_declaration.approval.prepare` / `approve` 及
  OpenClaw 工具 `oa_intellectual_property_declaration_approval_prepare`。能力仅接受精确
  标题族、模板、表单、节点和 CAP4 业务快照；申请人、类型、申报名称、发明/设计人、
  权属、申请用途和申请材料必须是只读展示，发现可编辑字段时在授权卡生成前拒绝。
- 填写卡只允许用户输入审批意见；授权计划冻结完整事项身份和业务快照，最终提交前重新
  读取并逐项比对。成功以同一 `affair_id` 从待办集合消失为准，未知结果不自动重试。
- 本轮开发和部署预检不提交该真实待办；真正审批仍需要用户在独立可信授权卡中明确确认。

### 15.62 2026-08-18 加班审批专用适配与 Workspace 漫游连接

- 辛国茂待办中的“【HR】加班申请审核单-郑其荣-0.65”提供了真实契约样本。只读探针
  确认模板 `-170938662527873499`、表单 `-213895278619572887`、“考勤审批”节点、
  `agree / disagree` 态度，以及申请人、申请时段、加班事由、直属上级选择和实际时段
  5 组业务字段。
- 新增 `oa.overtime.approval.prepare` / `approve` 及 OpenClaw 工具
  `oa_overtime_approval_prepare`。准备阶段先冻结精确事项、流程身份和业务字段，授权卡展示
  申请与实际时段及加班时长；最终提交前重新比对，并以同一 `affair_id` 离开待办列表为
  成功标准。真实提交仍需用户独立授权，本轮不自动处理当前待办。
- Agent Workspace 不再要求 Linux 服务器入站访问 Windows 工作站的当前局域网 IP。
  工作站新增仅回环反向 SSH 隧道和登录计划任务，服务器统一连接
  `ws://127.0.0.1:18789`；SSH 短断后自动重连，切换公司、家庭、酒店、有线或无线网络
  不再修改 AgentBridge 服务参数。服务器端口只监听回环地址，不扩展公网暴露面。
