# AgentBridge PoC 验证基线与阶段记录

> 状态：基础路线验证已通过，转入持续验收
> 更新日期：2026-08-19
> 目标：记录中心端零客户端连接器架构的 PoC 判据、阶段证据和历史范围。当前实现事实以
> [项目当前状态](../project-status.md) 为准。

## 0. 当前执行进度（2026-08-19）

中心端零客户端连接器 PoC 已覆盖四个真实系统、两个真实 OA 用户和三个用户端。PoC 的核心
技术路线已经验证，当前工作重点是稳定已有能力、逐流程补真实验收和推进生产化治理：
...
| 照明实验室测试系统 | 已完成读能力和 RTU 受控写一期/二期 | CAS 验证码登录、Cookie/JWT 会话、资产/告警/巡检/漏电读取、报告导出、RTU 备注、工区流转和精确处置已进入统一能力内核 |
...
| 多端任务 | 已完成一期与二期 | Agent Workspace、Telegram、微信共享用户主体、任务时间线、文本、卡片、图片和文件；支持自然语言跨端接续 |
| 移动端 | 已完成内网试点链路 | Android 已安装内部 CA；微信完成真实登录和读取，Telegram/微信均具备可信卡链接或内嵌入口；不同 WebView 的兼容性仍需持续观察 |
| 受控保活 | 已完成一期 | 按用户和系统记录活动时间，OA Cookie 会话与泰华 Token 会话分别探测、保活和刷新；真实失效仍要求重新登录 |
| 运行治理 | 已完成一期 | 独立管理控制台覆盖用户、会话、Token、任务、操作、审计和写暂停；版本化发布和只读发布验收已固化 |
...
[当前部署记录](../operations/current-deployment-plan.md)。
...
以下第 1 至 10 节保留最初 PoC 的问题、范围和判据，用于解释项目如何得出当前路线；其中的
“首期”“暂不实现”和阶段编号是历史计划，不应当作当前能力清单。
...
这些内容保存在 [目标架构](../architecture/agent-oriented-legacy-bs-adaptation-design.md) 和
[后续增强事项](../roadmap/deferred-considerations.md) 中。
...
首期采用一台位于目标系统可达网络区域的中心主机，不建设 Daemon 集群。能力内核、SQLite 账本、凭据代理、HTTP Session、浏览器 Worker 和 Profile 全部在该主机运行；最终用户设备不安装 Chrome 扩展、本地 Daemon 或连接器。旧 BSCLI Chrome 扩展、localhost Daemon 和代理型命令已彻底退役；迁移线索由 Git 基线和 [退役记录](../archive/legacy-bridge-retirement.md) 保存，不再保留可运行旧路径。
...
PoC 已通过，远程 MCP、移动端链路、多端 Workspace、管理控制台和四系统适配均已进入当前基线。
后续根据 [后续增强事项](../roadmap/deferred-considerations.md) 分阶段补齐生产能力，并继续为新增
业务能力保留本文件定义的真实验收、零越权和失败关闭原则。
