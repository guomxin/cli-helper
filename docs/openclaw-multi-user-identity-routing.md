...
目标是在同一个 OpenClaw Gateway 和同一个 Telegram Bot 中，让不同 Telegram
用户使用各自的 AgentBridge 身份、OA 登录态和权限，不共享 MCP Bearer Token，
也不共享浏览器 Profile。
...
- 已用两个虚拟 Telegram 用户完成 Token 隔离、并发请求、未知用户拒绝和
  会话串号测试。

目前还没有第二个真实 OA 用户，因此真实双用户登录、会话保活和 OA 并发验收
仍是待办项。现网保持原单用户配置，本次提交不会主动切换 Gateway。
...
Telegram 用户 B
  -> OpenClaw 可信 requesterSenderId=B
...
身份映射不使用聊天文本、模型生成参数或 Telegram 昵称。Telegram 昵称可以修改，
也可能重复，只允许使用宿主提供的稳定发送者 ID。
...
AGENTBRIDGE_MCP_TOKEN_USER_2002=abmcp_...
```
        ...
        "config": {
          ...
          "identityBindings": [
            ...
            },
            {
              "channel": "telegram",
              "senderId": "2002",
              "tokenEnv": "AGENTBRIDGE_MCP_TOKEN_USER_2002",
              "label": "用户B"
            }
          ]
        }
...
同一个 Gateway 连接多个 Telegram Bot 时，可在绑定中增加 `accountId`，形成
...
新的 `mcpUrl` 只提供地址，认证由插件按当前 Telegram 用户动态选择。配置了
...
- 同一 Telegram 用户的多个 OpenClaw 新会话：仍映射到同一 AgentBridge 用户和
  OA Session；
...
1. 为两名用户分别签发只读 Token，并配置两个 Telegram sender ID；
2. 两人分别从同一个 Bot 发起 OA 登录，确认各自页面显示预期 OA 姓名；
