...
| 知识产权申报审批单 | 暂未开放发起 | `oa.intellectual_property_declaration.approval.prepare` / `approve` | 固定模板 `-2986710992990286032`、表单 `-6972097064001584076` 和“审批”节点；冻结申请人、知识产权类型、申报名称、发明/设计人、权属、用途与申请材料，业务字段必须全部为只读；要求独立 `oa:write:approval` |
| 加班申请审核单 | 暂未开放发起 | `oa.overtime.approval.prepare` / `approve` | 固定标题族 `【HR】加班申请审核单-`、模板 `-170938662527873499`、表单 `-213895278619572887` 和“考勤审批”节点；授权卡展示申请人、申请与实际时段、事由、直属上级选择及加班时长；要求独立 `oa:write:approval` |
