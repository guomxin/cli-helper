...
> 文档状态：Approved v0.10，双用户跨端任务选择与受控接续已完成真实验收
>
> 更新日期：2026-08-03
>
> 现实起点：OpenClaw 2026.7.1、AgentBridge OpenClaw 插件 0.4.21、中心
...
- OpenClaw `0.4.21` 按 MCP Token scope 向同一用户的网页、Telegram 和微信会话注册同一套可用工具：
...
| 8、另一客户端继续且不重复 Operation | 2026-08-03 已完成辛国茂 Workspace -> Telegram、李世玉 Workspace -> 微信两组真实只读验收：均复用原任务、不重复待办列表、仅新增一个详情读取 Operation、零业务写入 |
