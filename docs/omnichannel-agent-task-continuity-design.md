...
> 文档状态：Approved v0.7，多端能力一致、有序文本同步和最终授权多端展示已实现
>
> 更新日期：2026-08-02
>
...
> 同步已经部署；跨客户端任务接续、共享模型上下文和 OBO 执行委托仍属于后续阶段。
...
- 按当前 Token scopes 裁剪模型可见工具目录；当前中央执行鉴权已经按用户隔离，
  但低权限用户仍可能看见随后会被中央服务拒绝的工具。
...
实施状态：部分实现。当前完成的是展示层同步，不等同于共享模型运行上下文。
...
- 从另一客户端选择并继续既有任务，以及向新 OpenClaw Session 注入裁剪后的任务摘要；
- 多个并行任务的选择与消歧；
- 跨端文件领取、断点和失败恢复；
- OBO 执行宿主切换；
...
| 8、另一客户端继续且不重复 Operation | 尚未实现任务接续与摘要注入 |
