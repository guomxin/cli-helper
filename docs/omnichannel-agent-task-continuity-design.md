...
> 文档状态：Approved v0.9，跨端任务选择与受控接续二期已实现
>
> 更新日期：2026-08-03
>
> 现实起点：OpenClaw 2026.7.1、AgentBridge OpenClaw 插件 0.4.16、中心
...
- OpenClaw `0.4.16` 按 MCP Token scope 向同一用户的网页、Telegram 和微信会话注册同一套可用工具：
...
- 两个真实用户分别完成一次“网页选择任务、伴随聊天端继续”的运行态验收；
- 跨端文件领取、断点和失败恢复；
...
| 8、另一客户端继续且不重复 Operation | 2026-08-03 已完成辛国茂 Workspace -> Telegram 真实只读验收：原任务复用、待办列表未重复、仅新增一个详情读取 Operation、零业务写入；李世玉真实接续仍待补 |
