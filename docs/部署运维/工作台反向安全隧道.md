...
- SSH 标准错误写入 `%LOCALAPPDATA%\AgentBridge\logs\workspace-tunnel-ssh.log`，不弹出重试窗口；
- 当前状态写入 `%LOCALAPPDATA%\AgentBridge\workspace-tunnel-status.json`。
...
若无线漫游、VPN 内部重连等情况没有改变网卡指纹，OpenSSH 保活负责兜底，最长约 20 秒
确认断线，再按普通 5 秒退避重连。服务器旧 `sshd` 尚未释放 `18789` 时，新连接虽然可以完成 SSH
认证，但会因 `Address already in use` 快速失败。守护器继续按 5 秒退避，直到旧监听释放，不需要
修改任何 IP 配置。此时服务器仍可能显示 `18789` 正在监听，但该旧监听背后的数据通道不一定可用。
