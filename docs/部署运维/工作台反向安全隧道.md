...
当前 `2、5、10` 秒受理前有界重试仍是线上实际行为。为避免较长的切网、VPN 重连或旧远端监听
释放过程直接导致用户请求失败，项目已形成
[工作台请求受理前可靠投递详细设计](../架构设计/工作台请求受理前可靠投递详细设计.md)。

该设计计划在中心端增加持久化的宿主受理等待区、就绪门控和单次受理核对。它尚未实现，不能据此
认为当前 17 秒等待已经升级为持久队列。未来实施后，本文件再更新实际参数、检查命令和发布基线。
