...
4. 新 `sshd` 子进程重新绑定服务器 `127.0.0.1:18789`；
5. AgentBridge 后续 Gateway 请求自动使用新隧道。

若无线漫游、VPN 内部重连等情况没有改变网卡指纹，OpenSSH 保活负责兜底，最长约 20 秒
确认断线，再按普通 5 秒退避重连。服务器旧 sshd 尚未释放 `18789` 时，新连接会快速失败，
守护器继续按 5 秒退避，直到旧监听释放，不需要修改任何 IP 配置。
...
| `remote port forwarding failed for listen port 18789` | 查找遗留的同目标 `ssh.exe`，再运行安装器 | 已有隧道占用远端端口；新版启动器会接管或在安装时定向清理 |
...
短暂空窗；一旦任务已被 OpenClaw 接受、已经发送工具调用或出现真实进展，就不做自动重放，
