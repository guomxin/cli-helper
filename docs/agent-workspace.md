...
- 在对话框粘贴、选择或拖入 JPEG、PNG、WebP 图片，作为当前消息的多模态输入；
- 查看当前用户的 AgentBridge 任务、事件时间线和关联端点；
- 从任务详情选择“继续任务”，在当前网页会话恢复权威任务上下文；
- 打开当前任务的可信登录、字段或授权页面；
...
- 用户在另一端明确引用“刚才网页端”等内容时，当前 OpenClaw Session 可读取同一
  用户其他端点的有界近期文本以解析指代。

当前二期不提供：

- 登录卡、字段卡的多端共同填写；
- 完整 Transcript 共享、任意隐式指代和 OBO 执行宿主切换；
- 企业 OIDC、找回密码和自助解绑。
...
- 跨端文本时间线只保存用户和助手的非敏感展示副本，不保存系统提示、工具参数、
  工具结果、凭据、Cookie、卡片字段值或授权决定；
...
1. 两个网页账户近同时发送带 XGM、LSY 唯一标记的“读取我的 OA 待办，只读不处理”；
2. 辛国茂返回 1 条待办，李世玉返回其账号下的 9 条待办，两张任务卡均进入“已完成”；
3. 两个网页的请求标记继续严格互斥，任务结果和 OA 主体没有交叉；
4. 辛国茂 Telegram、李世玉微信各确认 2 条 `timeline_message` 和 3 条
   `task_event`，证明用户请求、助手业务结果和任务终态均完成伴随端同步；
5. 验收后仍为 0 个活动任务、0 条待投递、0 条失败投递和 0 条身份一致性违规；
6. 李世玉结果约 43 秒，慢于辛国茂约 15 秒。日志确认其首个 `gpt-5.5` 模型请求
   在 21 秒后 `ETIMEDOUT` 并重试成功，不是跨用户串行队列或 AgentBridge 锁竞争。

2026-08-04 一期、二期双用户深度验收：

1. 两个网页账户近同时只读读取 OA 待办前 2 条，分别创建任务
   `d947836d-24da-4b5b-ab86-ec96d8fe3fbc` 和
   `91c5a46e-1828-4f72-8519-59e48566632e`；两项列表 Operation 均在约 2.3 秒内
   成功，辛国茂总数为 3、李世玉总数为 33，标题、发起人和 `affair_id` 没有交叉；
2. Task Hub 中两项任务都由网页端创建并进入终态；网页刷新前后各只有一张应用卡，
   文本和卡片没有重复，任务导航活动数回到 0；
3. 端点页严格显示辛国茂的网页 + Telegram、李世玉的网页 + 微信，未出现跨用户端点；
4. 辛国茂随后从 Telegram、李世玉从微信按显式 `taskId` 继续原网页任务。两项任务均
   没有新建第二个 Task，也没有重复待办列表；各只新增一个
   `oa.workflow.detail.get`，零业务写入；
5. 跨端指令和助手详情回复按中心时间线序号依次回到各自网页；全库没有重复
   `dedupe_key`，两组任务的 5 个 TaskEvent 投递均已确认；Telegram 与微信宿主日志
   分别确认文本发送成功；
6. 验收后两个 OA Session 仍为 `active`，4 个端点有效，0 个活动任务、0 条待投递，
   10 类身份一致性违规全部为 0；现存 2 条失败投递是 2026-08-03 旧版本产生的
...
只具备状态能力的端只收到状态消息。自 `agentbridge-interactions` 0.4.8 起，已绑定
端还共享用户和助手的非敏感文本展示副本；敏感字段值、凭据和工具内部消息不复制。
...
任务详情中的“继续任务”先在中心 Task Hub 为当前 Workspace Endpoint 选择目标
`taskId`，再向当前 OpenClaw 网页会话发送自然语言接续请求。浏览器不能提交
`userSubject`、Operation ID 或执行权限；中心端只允许选择当前网页登录身份拥有且属于
同一智能体宿主的任务。

Telegram、微信和 Workspace 中明确表达“继续刚才的任务”时，OpenClaw 使用宿主私有
MCP 解析最近任务。候选超过一条时只返回编号、标题、状态、来源端和更新时间，并将候选
顺序短期持久化；用户回复“第二个任务”后由中心按原候选集合选择，模型不能自行替换
`taskId`。选择结果包含由 Task Hub 和 AgentBridge 账本生成的非敏感权威摘要。

接续策略按任务现状收敛：

- 有待处理可信交互时，直接为当前 Endpoint 重显原 Interaction；
- Operation 正在运行时，只报告或观察状态，不发起第二次调用；
- 任务已成功、失败、取消或结果未知时，默认只报告权威终态；
- 用户明确要求查看详情、撤销、下载等新后续动作时，可沿用原 `taskId` 建立新的
  Operation，但仍经过 scope、可信交互、幂等和结果核验；
- 当前消息结束后，本地执行许可即清除，不能泄漏到下一条无关消息。

Task Hub 运行诊断同时统计接续状态，并检查 Endpoint、已选任务、候选任务的
`userSubject + agentHost` 一致性。诊断不返回聊天正文、业务字段或可信卡 URL。

## 网页应用卡与通知降噪

Agent Workspace 的对话区按 `taskId` 归并一次用户任务，但应用卡按 `interactionId`
稳定展示。同一用户指令只调用一个流程时通常只有一张卡；同时处理效能数据、周报等
多个独立流程时，任务列表仍只有一个任务，对话区会分别展示每个流程的字段卡和授权卡，
后一张卡不会覆盖前一张。卡片展示交互状态、影响、最新事件和对应操作入口；字段卡或
授权卡在另一端完成后，网页卡片通过 Task Hub SSE 自动更新到最终状态。刷新网页时，
进行中任务和最近六小时内的终态任务会从 Task Hub 重建，不依赖 OpenClaw 再生成
一条聊天回复。同一可信交互始终复用自己的卡片 DOM 节点和首次位置，后续状态只更新
该卡内容，不会把历史卡片删除后重新追加到对话末尾。

TaskEvent、跨端文本和应用卡更新共用 Task Hub 追加式时间线序号。SSE 首次连接从
当前最新序号建立游标，后续重连显式携带最后序号，不重放历史事件。普通任务进度
只更新应用卡，不再显示临时提示；只有失败和结果未知保留提示，同一异常会合并且
同时最多显示两条。
...
纯读取能力成功后，`task.operation.succeeded` 仍完整写入 Task Hub 和审计记录，但
Telegram、微信不再额外发送一条“操作已完成”的中间状态；用户只看到按序同步的原始
请求和智能体最终业务回答。受控写入、可逆写入、失败、结果未知和应用卡状态不适用该
静默规则，仍按原有治理链路展示。

网页、Telegram 和微信产生的用户/助手文本通过宿主私有
`agentbridge_host_timeline_append` 写入中心时间线。每条消息带稳定幂等键；同一
微信回复即使同时经过两个 OpenClaw 发送 Hook，也只发布一次。中心 Outbox 按时间线
顺序投递到其他消息端，网页通过 SSE 按同一序号读取。来源端不接收自己的镜像消息，
网页历史也不会与中心展示副本重复渲染。

微信直推受通道的“本次入站消息令牌”约束。若端点长时间没有新消息，OpenClaw 只做
一次直投尝试；令牌无效时，中心把消息保存为 `deferred`，不在几十秒内连续重试。
用户下一次从同一微信私聊发消息后，该端点的延后消息恢复为待投递并获得新的尝试
额度。恢复严格限定为同一 `userSubject + endpointId`，不会唤醒另一用户或另一通道。

网页收到 OpenClaw 模型连接错误时会在原消息位置显示失败原因。只有精确中止已经确认，
且事件流或会话历史能够证明尚未调用任何业务工具时，才提供“重新发送”；已经出现工具
调用、无法取得核验证据或无法确认中止的请求不提供安全重试，以免对写操作造成重复执行。

## Gateway 堵塞隔离与恢复

Workspace 不再把“HTTP 流断开”视为 OpenClaw Run 已结束。每个网页账号同一时刻只
允许一个智能体 Run；第二条请求直接返回“上一条仍在处理，本次没有排队”，不会在
同一 Session 后面形成不可见队列。每次发送前，BFF 先调用 `chat.abort` 清理该网页
专属 Session 的残留主 Run；如果 Gateway 连该只读预检都无法及时响应，本次请求不会
进入 `chat.send`。

`chat.send` 被接受后，BFF 持续保存 Run ID。出现以下任一情况时，必须按该 Run ID
调用 `chat.abort`：

- 首个真实生命周期、回答或工具事件出现后，执行等待达到 300 秒；
- 浏览器关闭、刷新或网络断开；
- Python 侧提前关闭 Gateway 事件生成器。

`chat.send` 返回 accepted 只代表 OpenClaw 已登记 AbortController，不代表模型运行已经
开始；`sessions.list.activeRunIds` 也来自同一登记表，不能单独作为真实启动证据。accepted
后 15 秒仍没有生命周期、回答或工具事件时，BFF 会按精确 Run ID 中止该次尝试并等待
Session 空闲。若 Run 已经从活动表消失，则直接进入证据核验，不再虚构一条启动进度。

第一次启动停滞后，BFF 使用 `chat.history` 查找 `${runId}:user` 幂等键，并检查该消息
之后是否出现 `toolCall`、`toolResult` 等工具痕迹。只有原 Run 已停止或已经消失、Session
已空闲且历史中没有工具活动时，才使用新的执行尝试 ID 自动恢复一次；历史读取失败、发现
工具痕迹或第二次仍无真实进展时均停止，不循环重放。恢复前后的两个 accepted 事件在网页
上迁移同一个临时进度区，不会产生两块“处理中”内容。

300 秒执行预算从首个真实进展事件重新起算，Gateway 排队或启动延迟不再提前吃掉业务工具
的执行时间。AgentBridge 主动中止时，OpenClaw 可能先广播一条 aborted 回声；客户端会
等待 `chat.abort` RPC 的确认结果后再给出具体错误，避免回声把工具活动和安全重试判定遮蔽。

只有 OpenClaw 明确确认该 Run 已中止，页面才显示“已安全中止”。如果无法确认，页面
和跨端时间线会显示保守失败提示；已经出现业务工具活动时，还会要求用户先核对业务
系统结果，不提供自动重试。失败文本使用稳定幂等键进入同一用户的中心时间线，因此
网页、Telegram 和微信按既有顺序看到同一结论，而不是某一端永久停在“进行中”。

OpenClaw 的插件工具描述缓存当前只存在于 Gateway 进程内存。Gateway 重启后的首轮
Agent Run 可能触发同步插件注册表冷载入，并在此期间阻塞 Gateway 主事件循环。发布
流程因此把“端口已监听”和“RPC 可连接”视为必要但不充分条件：

1. 配置 `diagnostics.stuckSessionWarnMs=30000` 和
   `diagnostics.stuckSessionAbortMs=120000`；
2. 深度 RPC 健康检查必须通过，CLI 与 Gateway 版本必须一致，插件不得有版本漂移；
3. 使用专用、无业务的 Warm-up Session 连续执行冷、热两轮 `READY` 探针；
4. 两轮都必须通过 Gateway 完成，且热路径不得超过 60 秒，发布才算就绪。

该门禁不修改 OpenClaw 源码，不会被 OpenClaw 升级覆盖。2026-07-31 在当前工作站的
真实探针结果为冷轮 19.326 秒、热轮 18.543 秒；探针没有调用 AgentBridge、OA、泰华
或语雀业务工具。

失败 Run 的进度块属于当前页面的临时状态，不属于持久历史。服务器时间线重新加载
成功后，或用户开始下一条任务前，页面会清除已经终止的临时失败块；真实用户消息、
助手回复和应用卡仍按中心时间线保留。这样旧的 `GATEWAY_TIMEOUT` 不会漂到后续成功
结果底部，也不会被误认为新任务仍在失败。

上述措施根治了 AgentBridge 侧的“超时后残留 Run 继续排队”和“受控重启后首个用户
承担冷启动”问题。OpenClaw 进程意外退出或工作站重启时，进程内工具缓存仍会丢失；
当前隔离、15 秒启动看门狗和 300 秒执行中止可限制影响，但不能消除 OpenClaw 自身的
同步冷载入。彻底消除该上游根因，需要 OpenClaw 将工具描述缓存持久化，或把插件发现
移出 Gateway 主事件
循环。本项目保持不修改 OpenClaw 源码，避免形成升级即失效的私有分支。
