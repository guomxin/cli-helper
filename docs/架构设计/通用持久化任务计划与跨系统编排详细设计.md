...
- 状态：一期内核与单写终点已验收；v2 语义入口、多来源、时间和保守完整性已上线，第一用户真实 Workspace 只读纵切已通过；
- 更新日期：2026 年 9 月 2 日；
- 所属阶段：跨系统编排一期；
...
2026 年 8 月 31 日后续真实请求暴露了关键词引导漏判、双来源汇聚缺失及单页查询口径问题。
后续入口和数据正确性改造以[组合任务语义规划与执行约束设计](./组合任务语义规划与执行约束设计.md)
为准；该补充已于 2026 年 9 月 2 日按宿主无关化、受理前可靠投递、Gateway 生命周期自愈和终态
事件排空的当前基线实现。第一用户自然表达只读纵切已经通过；多页历史穷尽、派生写入和双用户扩展
验收尚未完成，不应把当前有限纵切理解为所有自然说法和完整日期汇总已经通过。

版本关系固定为：

| 版本 | 作用 | 状态 |
| --- | --- | --- |
| `agentbridge.task-plan.proposal.v1` | 单来源 Binding、顺序计划和一个可信写入终点 | 已上线，继续兼容和恢复 |
| `agentbridge.task-plan.proposal.v2` | 共享语义策略、多来源、时间与完整性、私有结果投影和执行授权快照 | 已上线，第一用户真实只读纵切通过 |

本文继续作为计划内核和状态机的基础设计。v2 字段、自然语言入口和数据正确性以补充设计为规范来源，
不在本文复制第二套定义。
...
当前 `oa.workflow.done.list` 已接受可选的 `start_date`、`end_date`，但过滤只覆盖实际读取的页面
集合，不证明完整日期范围已经穷尽。`oa.workflow.sent.list` 的公共 MCP 入口尚未提供同等日期参数。
下面的 Proposal 是当前已办单来源 v1 结构；有日期参数不等于来源完整性已经满足正式汇总要求。
后续完整性合同、已发日期以及双来源绑定见补充设计，未发布的字段不能提前用于当前运行接口。
...
用户关键日期、集合或人员限制不被能力支持时必须说明缺口，不能仅把这些条件留在 goal 中就执行
无过滤查询；仅预览时未进入写入的工时等信息可以保留于目标说明。
...
是否需要计划由宿主按任务语义判断，不应由特定中文动词控制是否注入规则。当前插件的关键词引导
是已发现的实现缺口；后续将采用常驻规则和中央结构化调用约束，同时保留简单查询与原子填单路径。
...
v1 当前主要在计划创建时核对 Scope；v2 必须按补充设计保存不含密钥的执行授权引用，并在每个新
业务步骤、可信交互和 commit 前检查撤销、失效和缩权。计划持久化本身不是永久授权，也不能保存或
重放 Bearer Token。具体合同见补充设计“执行授权快照”。
...
### 15.1 当前实现的提交与恢复边界（2026-09-05）

- 新计划持久化独立的步骤键前缀 `task-plan:{planId}:{revision}`，步骤键再包含 `stepKey` 与尝试次数。
  内容 `planHash` 用于完整性核对，不用于把不同任务视为同一次执行。迁移前的在途计划保留旧键，
  恢复按原能力版本查找原 Operation，避免升级后重复读取或提交。
- 字段续办、授权续办和最终消费授权均复核原执行授权及能力、转换的冻结版本。当前 MCP Token
  有写权限不能替代原计划的全量授权；通过 MCP 新建的 v1 计划也保存执行授权快照。
- 取消和授权消费使用同一个 SQLite 写事务锁竞争。授权消费与 `commit_operation_id` 提交标记
  同事务持久化。取消先完成则旧卡失效、不能提交；提交先进入边界则返回 `PLAN_COMMIT_IN_PROGRESS`，
  只等待核验，不把已经开始的写入标记为取消。
- 重启后发现提交标记时，先核对对应 Operation。已核验成功就完成原计划；旧运行时未留下权威
  结果则进入 `outcome_unknown`。恢复不能重新调用提交工具。没有越过边界的步骤，只有冻结版本
  与原执行授权仍有效时才允许恢复。
- 提交应答或回读传输异常不等于业务确定失败。泰华日志、照明备注和中央提交兜底均保留
  `RESULT_UNKNOWN`；明确的业务拒绝与结果未知分别处理。

验证范围见[持久计划与受控写入边界修复验收](../验收记录/持久计划与受控写入边界修复验收-2026-09-05.md)。
...
- 有模型的宿主由模型提出 Proposal，Reference Host 用固定夹具验证同一 Proposal 合同；
...
计划步骤摘要与用户业务结果必须分层。管理端和公共计划状态只包含非敏感摘要；需要“先给我看”的
草稿由 v2 `PlanResultProjection` 作为同用户私有结果卡或受控模型结果交付。不能为了显示草稿把完整
来源或字段写进公共 Task Hub 事件，也不能让预览计划完成后只剩“2/2 步完成”而没有可查看成果。
...
后续“总结我昨天的OA已办和已发，时长为1小时，提交昨天的日志”请求未创建持久计划，而是直接
串联工具完成一次真实提交。提交与授权安全链路正常，但查询范围、双来源归纳和空结果正文存在问题。
本案例不推翻此前具体计划样本的成功证据，但说明自然表达和数据正确性尚不能整体宣告完成。

已形成独立[语义规划与执行约束设计](./组合任务语义规划与执行约束设计.md)和
[工作台验收方案](../后续规划/组合任务语义规划工作台验收方案.md)。范围为去掉关键词开关、
共享语义策略、中央 prepare 结构化门控、多输入绑定、来源完整性、空结果、私有结果投影、执行授权
快照与原子能力兼容。2026 年 9 月 2 日校准还纳入了受理后断线、计划创建后的中央续办和终态事件
排空边界。不增加新宿主、通用 DAG 平台、任意转换脚本或多写终点。

2026 年 9 月 2 日已完成运行实现和第一用户 Workspace 真实只读纵切。自然请求形成 OA 已办、已发和
合并转换三步计划；有限已办结果以 `partial` 警告完成预览，含写入终点的同类计划仍在写前硬停。
终态投影自包含受控事项，宿主不再通过 Operation 历史重建结果。随后单一待办查询仍走原子路径。
自动化和线上证据见[组合任务语义规划与执行验收](../验收记录/组合任务语义规划与执行验收-2026-09-02.md)。
