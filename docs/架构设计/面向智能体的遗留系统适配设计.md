...
> [项目当前状态](../项目当前状态.md)，阶段证据见 [PoC 验证基线](../验收记录/概念验证基线与阶段记录.md)，
> 暂缓事项见 [后续增强事项](../后续规划/后续增强事项.md)。独立用户网页端及网页、
> Telegram、微信之间的任务延续见 [多端智能体任务延续设计](./多端智能体任务接续设计.md)。
...
- 中央注册表发布 94 个业务能力：OA 51、泰华 5、语雀 4、照明 34；MCP 连同会话、
  可信交互、任务、文件和治理工具共 125 个工具；
- OA 已发布模板、待办、已发、已办、跟踪、详情、意见、证书文档和组织通讯录读取，以及出差、请假、
  补签、会议、普通协同、周报、效能数据、差旅费、劳动合同续签、知识产权、加班、离职、考勤确认
  和流程撤销等工作流能力；
...
- 照明系统已发布静态资产、当前运行态、RTU 告警、单灯告警、巡检、RTU 巡测和报告导出；
  旧“漏电”入口保留兼容，真实漏电只从 RTU 告警或巡测读取。受控写聚焦 RTU 告警备注、
...
当前运行证据见 [内网部署方案](../部署运维/当前内网部署.md)，能力验收状态见
[验收目录](../验收记录/验收记录导航.md)，写操作边界见 [受控写模型](./受控写入模型.md)。
...
- 手机和桌面上的同一用户可以查看自己的同一任务与操作状态，但设备身份不能扩大用户权限；跨端任务、客户端绑定和可信交互领取按 [多端智能体任务延续设计](./多端智能体任务接续设计.md) 实施；
