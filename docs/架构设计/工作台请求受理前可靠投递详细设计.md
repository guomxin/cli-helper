...
- `WorkspaceStore` 持久化 `agent_host_dispatches` 与 `agent_host_dispatch_events`；
- `WorkspaceApplication` 内置按账号保序的后台协调器和重启恢复；
- `OpenClawGatewayClient` 支持由中央协调器接管受理前重试，并禁止关闭订阅时中止已受理 Run；
- Workspace API/SSE 与前端支持刷新恢复、同键复用和受理前取消；
- Task Hub 运行诊断和管理控制台显示活动等待、连接恢复和受理不确定数量；
- 自动化已覆盖浏览器断开、受理前断线恢复、AgentBridge 重启和受理后断线不重投。
- 线上 Release `08d7ac09341a` 已完成正常投递和反向隧道无监听故障注入；等待期间按
  `1、2、3、5、8、10...` 秒退避，恢复后只产生一个 `accepted_run_id` 和一次真实宿主投递。
...
切换有线、无线、VPN 或重新联网时，反向端口交接可能出现两种短时中断：旧监听已经释放而新监听
尚未建立，或者旧 `sshd` 会话仍占用服务器 `127.0.0.1:18789`、实际数据通道可能已不可用或无法
证明可用，新会话因 `Address already in use` 无法接管。改造前的 Workspace 只在当前 SSE 调用栈
中按 `2、5、10` 秒执行有界重试；17 秒内恢复通常可以继续，超过该窗口则安全失败，请求不进入
业务系统。

本期实现不以“更快重启 SSH”作为主要方案，而是在 AgentBridge 中央端增加持久化的“宿主受理等待区”：
...
SSH 协议在这里没有 AgentBridge 意义上的“租约”。本文原先使用的“隧道租约空窗”容易与 OA
登录会话、MCP Token、协调器 Lease 或 Worker 业务租约混淆，统一改称“反向隧道端口交接中断”。
它包括两种形态：

1. **无监听空窗**：旧 `sshd` 已释放 `127.0.0.1:18789`，新会话尚未完成绑定；
2. **旧监听占用冲突**：旧 `sshd` 仍持有监听，但对应 SSH 数据通道可能已经失效或不可可靠使用；新
   SSH 会话可以连接并完成认证，却因端口已占用而无法建立新的反向转发。

第二种形态下，`ss` 仍能看到监听，甚至 TCP 建连可能成功，但请求无法真正穿过旧 SSH 会话到达
Windows OpenClaw Gateway。因此“端口存在”不是宿主链路就绪的充分证据。
...
在最后一步完成前，AgentBridge 无法可靠地通过该地址访问 OpenClaw Gateway。旧服务器进程释放端口
稍慢时，新 SSH 会遇到 `remote port forwarding failed for listen port 18789` 或服务端
`bind [127.0.0.1]:18789: Address already in use`，随后按现行守护策略继续重连。
...
它的不足是等待只存在于当时的 HTTP/SSE 调用栈中。等待窗口耗尽、AgentBridge 重启或浏览器连接中断后，
请求没有一个独立、可恢复的受理前状态实体。

### 2.3 需要解决的用户问题

用户不应为了几秒至几十秒的切网或端口交接中断承担以下成本：
...
| `attempt_count` | 真正开始向宿主发送请求的次数 |
| `retry_count` | 宿主受理前连接恢复和安全退避轮次 |
...
默认 60 秒截止。此窗口覆盖现行网卡 2 秒检测、最多约 20 秒 OpenSSH 静默断线确认、旧远端监听
释放和新会话接管期间的 5 秒重连周期，同时避免请求在用户不知情时长时间滞留。

## 十、就绪探测

### 10.1 两层探测

1. **路径可达**：通过实际反向转发执行有界 HTTP/WebSocket 握手并收到响应，不能只用 `ss` 或 TCP
   `connect()` 判断监听存在；
2. **语义就绪**：调用宿主只读控制面，例如 OpenClaw `system.info`，确认 Gateway 已完成启动、协议
   可用且身份绑定可以解析。

旧 `sshd` 监听残留时，第一层必须把握手超时、连接复位或无有效响应判为不可达。只通过端口监听
检查不能直接投递，因为监听可能对应失效的旧 SSH 通道，本机 Gateway 也可能尚未启动。第二层不读取
OA、泰华、语雀或照明系统，不触发模型运行，也不创建 Task。
...
| 错误 | 示例 | 是否等待 | 处置 |
| --- | --- | --- | --- |
| `HOST_TRANSPORT_UNAVAILABLE` | 无监听、旧监听路径超时、端口交接冲突、连接拒绝 | 是 | 转 `waiting_host` |
...
核心状态只保存宿主无关的 `HOST_TRANSPORT_UNAVAILABLE`。运维日志和指标可以附带低敏诊断原因
`listener_absent`、`path_stale`、`port_handoff_conflict` 或 `connection_refused`，但这些 SSH 部署细节
不得进入跨宿主协议，也不得展示给普通用户。
...
- `host_transport_failure_total{reason}`，其中 `reason` 仅使用受控枚举
...
能够更快发现部分断线，但不能消除旧远端监听释放和新监听绑定之间的竞争，也不能保证残留监听背后的
数据通道仍然可用，更不能解决请求持久化、幂等受理和页面刷新。因此只能作为隧道守护优化。

### 18.3 双端口隧道加稳定代理

可让新隧道先绑定备用端口，健康检查通过后由服务器代理原子切换，再关闭旧隧道。这是
make-before-break 路线，能够显著缩短端口交接中断，但需要双槽位、健康检查、代理配置、旧连接清理
和 split-brain 防护。
...
| 旧端口占用导致新会话连续报 `Address already in use` | 即使监听仍存在，路径握手也判为不可用；请求保持 waiting，释放后受理一次 |
...
受控终止当前隧道 SSH 子进程测试；旧监听占用冲突由隔离端口或模拟适配器复现，不能为了制造冲突而
破坏正式 `18789`。受理后断线沿用现有同 run 核对验收。真实写能力只验证正常链路，故障边界由
模拟适配器证明。
...
本设计实现完成必须同时满足。以下条件已于 2026-08-30 的自动化、发布冒烟和真实故障注入中满足：
...
- `HOST_ACCEPT_TIMEOUT` 主要由远端端口交接冲突造成，而不是实际断网；
