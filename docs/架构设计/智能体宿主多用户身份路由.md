...
导出目录当前包含 90 个 AgentBridge 代理工具描述。OpenClaw `0.4.49` 在运行时进一步
...
令牌仅用于认证；业务能力统一在管理控制台按用户配置，未配置时默认无业务权限。签发命令不再接受 `--scope`。
