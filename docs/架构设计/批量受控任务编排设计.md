...
- 状态：2026-09-07 已扩展通用待办批量；下文二至十四节保留补签一期设计，当前通用契约以第十五节为准；
- 当前范围：多个已支持类型的待办，支持精确选择与筛选冻结，不限定两条；
- 当前对外工具：`oa_workflow_pending_batch_prepare`，旧 `oa_missed_punch_approval_batch_prepare` 保留兼容；
- 通用默认策略：`stop_on_failure`，默认上限 20 条，请求可设置 1 至 100 条，超限拒绝而非截断；
...
4. Agent Host 使用跨协调器实例共享的续办占位，同一 Interaction 只有一个执行者可以申请协调租约；
5. 父任务进入终态后，宿主主动终止该任务的残留轮询并清除恢复记录，中心端也拒绝再签发协调租约。

第三层专门处理批次条目并发：Workspace 和 Telegram 可能几乎同时看到同一张卡完成，第二个迟到回调不能
把“第一条成功”记到第二条。`record_batch_item_activity` 也会识别已经绑定旧条目的 Operation，拒绝污染
当前项。第四、五层处理宿主竞态：主调用链、伴随端通知和重启恢复即使同时看到同一张已完成卡片，也只能
续办一次；若父任务已经成功、失败或取消，迟到恢复会被安静忽略，不产生 `RESUME_FAILED`，也不进入业务系统。
...
| 父任务终态后收到迟到交互通知 | 清理残留轮询和恢复记录，中心端返回 `task_terminal`，不报错、不续办 |
...
2026-09-07 用户要求处理两条研发中心效能流程，宿主只创建了一个单项审批；第一条成功后没有持久批次可以推进。
这不是“两条”的特殊问题，也不是 Workspace 成功后不再唤醒模型的策略错误。让模型再次猜测还有哪些待办不可靠，
因此把已存在的补签批次账本扩展为按条选择专用能力的通用批次，继续通过中央可信交互协议续办。

### 支持的类型

补签申请、效能数据、差旅费审批报销、劳动合同续签、知识产权申报审批、加班审批、离职审批、月度考勤确认、
周报阅办和普通协同，共十类。标题只用于挑选候选能力；生成授权前仍验证每类原有模板、表单、节点策略和字段。
普通协同不是任意表单审批的兜底入口；预检不通过的事项停止在该条，不绕过校验。

### 选择与冻结

公开输入为 `affair_ids`、`workflow_types`、`keyword`、`max_items` 和幂等键，均不要求普通用户手填。
“上面的、选中的、第几条”由宿主根据已读取结果传入精确 ID，保持原选择顺序；“当前所有某类事项”使用类型和
标题关键词筛选，多个筛选条件取交集。服务端重新核对这些 ID 是否仍属于当前用户待办。

必须先取得完整待办来源。分页或数量导致 `coverage.status` 不完整、目标消失、选择不一致、范围包含未支持类型、
超过上限时均不开始审批，并返回明确错误。通用入口默认上限 20，可显式提高到 100；不把前 20 条冒充全部。
当前读取预算最多 1000 条，不能证明完整时停止，不无限遍历。

每条冻结事项标识、展示摘要、来源指纹、专用 prepare/commit 名称及版本。新出现的待办不加入在途批次；
一个用户的相同目标不能同时进入通用批次与旧补签批次。旧补签入口的上限语义保留兼容，新扩展不依赖它。

### 执行与恢复

1. 复用一个父任务和既有批次表，不新增数据库迁移；生成当前条字段卡，展示第几条、总数和事项摘要。
2. 每条独立字段填写、独立授权。字段可填写性、专用预检及单项写入后的权威回读保持原契约。
3. 只有当前条权威成功才能推进下一条；准备和提交再次校验冻结能力版本及治理暂停，不扩大 Token 权限。
4. 下一条使用稳定幂等键。重启后使用持久清单；提交已成功但尚未写入批次进度时，重复恢复只补进度，不重复提交。
5. 旧条目的迟到回调返回当前条最新交互，不能重发当前条已经填写完的旧卡，也不能跳过未完成事项。
6. 拒绝、过期、明确失败、结果未知均停止后续项；已成功项保留，不自动回滚或重新审批。

通用能力为 `oa.workflow.pending.batch.prepare`，权限沿用 `oa:write:approval`；单项 commit 仍不向模型开放。
OpenClaw 插件只增加公共目录和汇总展示，不修改 OpenClaw 源码；Reference Host 可使用同一 MCP 入口和恢复协议。
不引入靠自然语言关键词触发的服务端路由，也不把批量审批强行装入跨系统任务计划。

### 多端体验与验收

Workspace 卡片显示批量进度，即使当前交互有自己的说明也不会遮住该进度。Telegram/微信沿用既有可信交互协调器
接收当前卡和最终汇总；全部完成与部分成功后停止分开表达，不唤醒模型重复审批。
...
核对选择范围后逐张操作；中间不发“继续”。总数不限两条。拒绝其中一张时，余下项应停止，成功项不重复。
线上真实审批仍由用户在卡片决定；自动化模拟数据不等同于真实 OA 业务验收。
本轮证据见[通用待办批量处理验收](../验收记录/通用待办批量处理验收-2026-09-07.md)。
