...
一个可独立观察和收敛的用户目标对应一个 `taskId`，可以关联多个 Operation、Interaction 和
Artifact。宿主不得按“每张卡一个任务”拆散业务上下文。若同一轮先查询候选对象、再进入审批、
填报或撤销，查询可以归属用户轮次任务，但每个受治理业务 `*_prepare` 必须建立独立写任务；这是
从“定位对象”切换到“承担业务副作用”的任务边界，不是按卡片拆任务。该写任务一旦建立，字段卡、
授权卡、提交和权威回读都复用同一 `taskId`，终态后宿主立即释放绑定。登录恢复仍归属被中断的
读取任务，`agentbridge_task_plan_prepare` 仍归属持久组合计划，不适用这条独立写入口规则。
