...
| 基线项 | 当前事实 | 对一期设计的影响 |
| --- | --- | --- |
| OpenClaw | `2026.7.1`，AgentBridge 插件 `0.4.62` | 以现有插件行为和测试为迁移输入，但不把历史偶然行为直接定义为标准 |
...
截至 2026 年 8 月 29 日，一期代码已完成：

- `schemas/agent-host/v1/` 发布 12 份机器 Schema 和 H01-H25 共享测试向量；
...
- OpenClaw 插件 `0.4.62` 按相同 Profile、上下文和 Lease 工作，并保留 Telegram、微信和
  Workspace 的渠道适配职责；
