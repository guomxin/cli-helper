...
6. 宿主重启后等待任务可恢复，已越过副作用边界的操作不重放；
7. 多端显示同一卡时只产生一次有效业务动作；
8. L1 宿主遇到可信交互明确停止且不泄露秘密；
9. OpenClaw 插件通过共享契约测试，当前三端体验无回归；
10. 自动测试、远程冒烟、双宿主真实验收和文档链接检查全部通过；
11. 一期没有安装或修改 Goose、nanobot、LibreChat、Open WebUI 等外部宿主。
