...
### 0.2 2026-07-24 双用户与接收处理增量

- 远程 MCP 和 OpenClaw 已接入两个真实 OA 身份：Telegram 绑定
  `guomao` / 辛国茂，微信绑定 `lishiyu` / 李世玉。两者使用独立 Token、预期
  下游身份、中央会话、Profile 和操作账本分区；
- 两个会话均由实时 OA 探测确认 active，同一时段读取到不同的真实待办集合。微信
  只读请求和 Telegram 写请求分别回到各自原始私聊，没有跨通道续接；
- `CentralBrowserWorker` 在使用 Profile 前统一执行 `chmod 0700`，同时覆盖历史目录；
  线上两个真实 Profile 已完成权限复核。无法限制权限时 Worker 失败关闭；
- 普通协同接收处理已完成真实“字段卡 -> 独立授权 -> 提交 -> 待办消失回读”闭环，
  目标为“关于征集济南市大数据产业专家入库工作的通知”，结果明确为
  `workflow_approved=true`；
- 当前实现证明了同一中心服务账户内的应用身份、Token、会话、Profile、账本和通道
  路由隔离。每用户独立 OS/容器 Worker 仍未部署，因此下载、截图、Cookie 和日志的
  跨安全主体不可读性仍是生产化门槛，不能把本次结果表述为完整生产隔离。
...
5. **遗留登录使用受控的可信交互。** 登录过期时，服务端生成绑定用户、系统和会话的一次性 `AuthChallenge`。普通登录字段通过凭据代理直达中心浏览器；滑块、短信验证等必须由人操作的页面采用登录专用的可信交互浏览器卡。两种模式都不向模型暴露账号、密码、验证码、Cookie、浏览器端点或控制令牌；USB Key、ActiveX、客户端证书和电子签章仍需专门接管方案或明确拒绝。
...
- 支持同一用户通过网页、Telegram、微信等智能体客户端延续同一 `taskId`，并在一个客户端发起、另一个受信客户端确认关键节点；
...
- 重建遗留系统原有表单、审批和业务菜单；独立 Agent Workspace 只承载智能体对话、任务状态和可信交互入口，不实现另一套遗留系统业务前端；
- 将所有遗留页面自动转化为稳定 API；
...
本项目不重建遗留系统业务前端。普通用户可以使用独立 Agent Workspace、既有智能体宿主的确认界面或企业审批服务查看任务并进入可信交互，但这些界面不能复制下游业务规则，也不能绕过 AgentBridge 能力、身份、授权和审计边界。模型本身不能生成、代替或确认执行授权。
...
账号、密码、验证码和 MFA 值通过认证卡片到凭据代理的独立可信通道传输，生产环境必须使用 TLS；受控内网 PoC 可通过显式开关临时使用固定私网 IP HTTP，必须保持在公司内网且不得做公网映射，主机防火墙或上游 ACL 可在后续加固，该例外不属于生产方案。普通模型只看到 `requires_user_action`、`challengeId` 和非敏感状态。秘密不得出现在聊天记录、MCP Tool 参数、模型上下文、Trace、普通日志、推送、截图、HAR、剪贴板或分析埋点中。默认只在 Worker 内存中短暂使用并在登录完成后清除；只有用户明确同意且生产凭据保险库、轮换、撤销和审计均已启用时才允许持久保存。
...
用户名密码、短信/TOTP 验证码和 MFA 推送可以通过多阶段字段型认证卡处理。对于滑块和必须在原登录页完成的短信流程，可使用数据盲的交互式登录卡：中心 Broker 为每个挑战分配独立 Xvfb、Chromium Profile、回环 RFB/CDP 和不透明 noVNC 路由，卡片嵌入原生浏览器画面与输入流；一次性 VNC 密码由卡片自动带入且不进入 HTTP 请求、日志、模型或 MCP 结果。登录后必须核验下游身份、加密保存会话并销毁全部临时资源。语雀一期已经采用并真实验收该模式，但不自动破解滑块。二维码同机扫码、USB Key、ActiveX、客户端证书和电子签章仍转入受控桌面接管或明确标记为不支持。
...
- 智能体宿主先建立稳定 `taskId`，并在首次写请求前生成、持久化 `requestId/idempotencyKey`；调用应快速获得 `operationId` 并关联到原任务。手机 App 切网、挂起或关闭不影响后台操作，重连后先恢复任务，再通过 `operations.get/watch` 核对当前业务操作；
