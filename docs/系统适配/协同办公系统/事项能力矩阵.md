# OA 事项能力矩阵

原始矩阵日期：2026-06-29
当前中心能力快照：2026-09-08

## 目标

`oa matter matrix` 把历史事项归类、模板匹配和写动作治理状态整理成一张只读能力表。它的用途不是执行 OA 写动作，而是帮助智能体先判断某个事项目前能做什么、还缺什么验证。

## 数据来源

- `oa history profile`：从已发、已办、跟踪等历史事项中聚类高频事项。
- `oa template list` / `template_list_api`：从模板中心 REST 读取可发起模板，包含 `template_id`、`form_app_id`、`category_name`、`module_type`、`body_type`。
- `oa template match`：把历史事项聚类匹配到可发起模板。
- `oa matter profile`：把匹配结果整理为事项目录。

`oa matter matrix` 只消费 `oa matter profile` 的结果，不打开模板发起页，不读取当前待办详情，不调度浏览器写任务。

## 命令

```bash
python -m bscli.cli.main --home .bscli oa matter matrix --kind all --limit 50
python -m bscli.cli.main --home .bscli oa matter matrix --kind all --keyword 报销 --fields matter_id,name,coverage_status
```

返回结构为 `bscli.oa_matter_matrix.v1`，核心字段包括：

- `coverage`：整体覆盖统计，例如可发起草稿的事项数量、模板未匹配数量。
- `items[].launch_handling`：发起处理能力，当前重点是是否具备 `launch_dry_run` 和 `launch_save_draft` 的安全路径。
- `items[].received_handling`：接收处理能力，当前统一表达为需要当前待办项后再走 `matter preflight`。
- `items[].coverage_status`：事项当前能力状态，例如 `launch_ready_received_preflight_ready` 或 `needs_template_match`。

## 当前中心能力快照

2026-09-07 增加 `oa.workflow.pending.batch.prepare`：补签、效能、差旅费、劳动合同续签、知识产权、加班、离职、
考勤确认、周报与普通协同可复用专用接收能力组成冻结批次。批量不是统一同意所有表单，每项仍需独立授权和预检。
完整契约见[批量设计](../../架构设计/批量受控任务编排设计.md#十五通用待办批量扩展)。

旧 `oa matter matrix` 是退役桥接时期的只读发现产物，继续作为模板匹配和
迁移线索，但不再是当前执行入口。当前智能体应以中心 Capability Registry
和 MCP 工具目录为准。2026-07-22 快照包含 33 个 OA 能力，其中 7 个只读、26 个
受治理写阶段；当时中心 MCP 总计 40 个工具。当前中心工具数量和模型可见目录已经继续
扩展，实时基线见 [当前内网部署](../../部署运维/当前内网部署.md)，不要把本历史快照用于
校验当前 OpenClaw 工具总数。待办、已发、已办、跟踪是四个独立集合，不得互相替代：

- 待办读取首页 `pendingSection`；
- 已发读取完整列表页 `listSent` / `getSentList`；
- 已办读取完整列表页 `listDone` / `getDoneList`；
- 跟踪读取首页“跟踪事项”的独立“更多”页
  `portalAffairController.do?method=moreTrack`，网格契约为
  `gridId` / `getMoreList4SectionContion`，精确事项 ID 来自行复选框值。

2026-07-26 真实只读验收中，辛国茂账号待办为 0 条；已发总数 215、当前页
50 条；已办总数 1025、当前页 50 条，当前两页 `affair_id` 交集为 0；
独立跟踪页总数 28、当前页 28 条。当前加载范围的交集为：已发/已办 0、
已发/跟踪 5、已办/跟踪 0。已发和已办目前读取 OA 首屏 50 条，跟踪事项当时
只有一页；各集合的 `total` 用于呈现规模，不冒充已经遍历未加载页面。

| 事项 | 发起处理 | 接收处理 | 当前证据与限制 |
|---|---|---|---|
| 出差申请单 | `oa.business_trip.prepare` / `save_draft`；独立的 `submit.prepare` / `submit` | 暂无专用接收能力 | 草稿已做真实保存回读；2026-07-22 真实提交与撤销均成功，提交采用权威已发网格和详情双回读；正式提交要求独立 `oa:write:submit` |
| 乘坐飞机申请单 | 暂未开放发起 | `oa.flight_application.approval.prepare` / `approve` | 2026-09-23 从当前待办只读提取独立契约：固定标题族 `【综合】乘坐飞机申请单`、模板 `-2302881233144493310`、表单 `2631521264809559746`、`approve`/`审批` 节点。冻结申请人、工号、部门、申请日期和每条含记录标识的航段（出发时间、出发地、到达地、事由）；业务字段必须只读。意见填写卡先展示已校验的完整航段和现有意见信息，授权前再次读取；详情变化时授权卡提示重新核对。每条需独立授权；零写预检与正式审批结果分别验收。 |
| 请假申请单 | `oa.leave.prepare` / `save_draft`；独立的 `submit.prepare` / `submit` | 暂无专用接收能力 | 仅开放 `年休`、`事假`、`调休`；2026-07-22 真实提交与撤销均成功；正式提交要求独立 `oa:write:submit` |
| 补签申请单 | `oa.missed_punch.prepare` / `save_draft` | `oa.missed_punch.approval.prepare` / `approve` | 独立发起与接收状态机；审批冻结模板、表单、节点策略、同意态度和业务字段，执行前重新校验；不暴露通用 `ContinueSubmit` |
| 劳动合同续签表 | 暂未开放发起 | `oa.labor_contract_renewal.approval.prepare` / `approve` | 固定模板 `3868679303223263344`、表单 `6514522401641018463`；确认卡展示员工、合同期限、综合评价、续签建议、指导意见和反馈，业务字段只读时不得由智能体改写；要求独立 `oa:write:approval` |
| 知识产权申报审批单 | 暂未开放发起 | `oa.intellectual_property_declaration.approval.prepare` / `approve` | 固定模板 `-2986710992990286032`、表单 `-6972097064001584076` 和“审批”节点；冻结申请人、知识产权类型、申报名称、发明/设计人、权属、用途与申请材料，业务字段必须全部为只读；要求独立 `oa:write:approval` |
| 加班申请审核单 | 暂未开放发起 | `oa.overtime.approval.prepare` / `approve` | 固定标题族 `【HR】加班申请审核单-`、模板 `-170938662527873499`、表单 `-213895278619572887` 和“考勤审批”节点；授权卡展示申请人、申请与实际时段、事由、直属上级选择及加班时长；要求独立 `oa:write:approval` |
| 离职申请单 | 暂未开放发起 | `oa.resignation.approval.prepare` / `approve` | 2026-08-26 从真实待办提取专用契约；固定标题族 `【HR】离职申请单-`、模板 `3483439346772952417`、表单 `9167110384557358951` 和“审批”节点；冻结员工、工号、部门岗位、入职与离职日期、岗位类别、是否有证书、证书名称、离职原因和手写申请附件，业务字段必须全部为只读；选择“有证书”却读取不到证书名称时失败关闭；要求独立 `oa:write:approval` |
| 工作交接单 | 暂未开放发起 | `oa.work_handover.approval.prepare` / `approve` | 固定模板 `-1377138047593329703`、表单 `5247761788960957082`、审批节点；支持标题的自动发起前缀。冻结移交人、部门、日期、监交人、实际交接类别、全部已读取工作/财务明细及确认意见；表单有可编辑字段、未知字段或明细不完整时拒绝。纳入逐项授权批量；2026-09-08 中央会话零写预检通过，真实提交待授权验收 |
| 月度考勤确认单 | 系统自动发起 | `oa.attendance_confirmation.prepare` / `confirm` | 固定模板 `-7231800401165464345`、表单 `5072944770639779741` 和“发起人填写”节点；提交计划冻结员工月份、出勤统计及 OA 已选的有无异议结论，业务字段只读时不得由智能体改写；生成填写卡前先验证流程契约 |
| 新建会议 | `oa.meeting.create.prepare` / `create` | 当前中心端未开放会议邀请回复 | 已完成真实创建发送与会议室占用、会议详情双回读；填写、授权前和写边界前均复用会议室域的实时空闲查询 |
| 会议室申请（非事项） | `oa.meeting_room.availability.list` / `my_applications.list`；`application.prepare` / `application.create`；`application.cancel.prepare` / `application.cancel` | 不适用 | 独立读取会议室目录、占用和“我的申请”；可申请一间会议室并精确撤销本人的独立申请；不创建或撤销关联会议；提前结束、催办和审核尚未开放 |
| 效能数据流程 | 不适用 | `oa.efficiency_data.approval.prepare` / `approve` | 绑定标题族、自由协同字段结构、审批节点和流程身份；当前两条待办已完成零写页面探针，真实审批待用户逐条授权 |
| 差旅费审批报销单 | 暂未开放发起 | `oa.travel_expense.approval.prepare` / `approve` | 固定模板 `-2046021869351779722`、表单 `-2571419096251022663`；授权卡展示金额、关键归属字段和附件数，不展示收款账号；真实审批待用户授权 |
| 周报发送流程 | 人工发起入口暂未开放 | `oa.weekly_report.acknowledgement.prepare` / `acknowledge` | 固定模板 `1610567580409022440`、表单 `-2351708227632217917`；OA 节点是“知会”而非审批，提交时不伪造同意态度；真实阅办待用户授权 |
| 普通协同事项 | 暂未开放发起 | `oa.standard_collaboration.approval.prepare` / `approve` | 仅允许无模板、无专业表单字段的普通协同，显式排除 HR、报销、采购、用印、效能数据和周报标题族；当前通知事项已完成零写页面探针 |
| 已发流程撤销（跨事项） | `oa.workflow.revoke.prepare` / `revoke` | 不适用 | 独立 `oa:write:revoke`；仅接收已发列表返回的精确 `affair_id`，强制填写撤销附言和单独授权，以已发消失且同一事项回到待发撤销态为成功标准；2026-07-22 已完成出差与请假测试流程的真实撤销闭环 |
| 其他历史事项 | 仅保留模板/历史发现证据 | 仅保留只读预检证据 | 未形成工作流专用中心能力前不得借用相似表单的底层提交动作 |

这里的 `prepare`、`save_draft`、`submit`、`approve` 和 `create` 是面向
智能体的业务能力阶段，不是让智能体直接拼接 OA API。每个事项可以拥有不同
的底层接口、字段和成功判据；能力矩阵只表达可用边界，不把原子动作对外暴露。

## 安全边界

- 发起侧：矩阵只给出 `oa launch dry-run` 和 `oa launch save-draft --confirm` 的下一步命令建议，不自动打开页面、不自动保存草稿。
- 接收侧：已注册事项可走“字段卡 → 精确事项授权 → 单次处理 → 待办消失回读”；每次授权只绑定一个 `affair_id`。未注册事项仍只允许只读预检，不得借用其他流程的提交动作。
- 模板未匹配时：矩阵标记为 `template_unmatched`，后续应先修正模板匹配或补采样。
- 会议类事项：会议创建和会议室申请是两个独立能力域，共用会议室读取事实源和
  `oa:write:meeting` 权限，但使用不同冻结计划、字段卡和授权卡；两者都不能套普通协同模板执行器。
- 已发撤销：是跨事项的独立受治理能力，不属于某个表单的发起或接收原子动作，也不得自动用于测试数据清理。OA 可能产生通知、审计和表单业务副作用。

## 后续扩展

1. 对高频普通协同事项补发起页字段画像，优先验证保存草稿路径。
2. 对当前待办中出现的目标事项做 `matter preflight`，再决定是否提升接收处理动作。
3. 将会议发起的 direct-create 验证状态回填到特殊事项画像，并继续为补签等特殊入口建立画像。
4. 每提升一个写动作，都把验证状态回填到矩阵，而不是只新增底层原子命令。
