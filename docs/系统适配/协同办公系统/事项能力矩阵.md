...
| 乘坐飞机申请单 | 暂未开放发起 | `oa.flight_application.approval.prepare` / `approve` | 2026-09-23 从当前待办只读提取独立契约：固定标题族 `【综合】乘坐飞机申请单`、模板 `-2302881233144493310`、表单 `2631521264809559746`、`approve`/`审批` 节点。冻结申请人、工号、部门、申请日期和每条含记录标识的航段（出发时间、出发地、到达地、事由）；业务字段必须只读。每条需独立授权；零写预检与正式审批结果分别验收。 |
