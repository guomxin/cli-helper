...
| 会议室读取（非事项） | `oa.meeting_room.availability.list` / `my_applications.list` | 不适用 | 独立读取会议室目录、容量、审核要求、指定时段占用及“我的申请”；不是会议创建，也不是 OA 已发事项；会议室撤销、提前结束和催办尚未开放 |
...
- 会议类事项：会议创建和会议室读取是两个独立能力域。会议创建复用会议室读取的事实源，
  但不能套普通协同模板发起执行器；会议室申请写操作也不得借用会议创建授权。
