...
| 部门信息库（语雀） | 已完成真实验收 | 公共区知识库、跨库目录和组织搜索已固化；Doc、Sheet、Table 与图片文档均完成结构化只读验收，支持表格行分页；隔离 noVNC 登录、账号核验、加密 Cookie 保存和临时进程清理均通过；搜索摘要不返回，正文执行凭据和 Token 脱敏 |
...
- 已识别单组织公共区、知识库、跨库目录、组织全文搜索和统一文档读取接口，并将页面探索结果固化为四个只读业务能力；
- 读取扩展保持同一业务能力表面：目录默认跨全部知识库并支持类型、更新时间、排序和分页；搜索默认组织范围；文档读取在适配器内统一解析 Doc、Sheet、Table、正文表格、图片 OCR、链接和附件元数据；
...
- Release `871db364eba0` 完成真实结构化读取验收：跨库目录 146 篇、组织搜索“设备”27 条；普通 Doc 表格、Sheet 行分页、Table 只读记录和含 4 张图片的文档均返回预期结构。当前真实样本未发现可验收的独立附件卡，因此只返回附件元数据，不宣称支持附件下载。
