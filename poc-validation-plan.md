...
| 项目 | 状态 | 证据 |
|---|---|---|
| 能力目录与 Schema | 已扩展 | 当前注册 18 个 OA 能力：6 个只读能力和 12 个受治理写阶段；中心 MCP 共 25 个工具。字段卡、授权卡及内部 OA 标识仍只通过不透明 ID 对智能体暴露 |
...
| Streamable HTTP MCP | 已完成内网单用户真实纵切 | 官方 MCP SDK 无状态 JSON 传输；当前中心运行时发布 25 个工具。独立服务进程、官方客户端和 OpenClaw 均完成 HTTPS 握手、Bearer 绑定与真实 OA 调用；CLI/MCP 同幂等键返回同一操作 |
...
| 出差申请正式提交 W2 候选 | 已实现并完成零写入真实预检 | 独立 `submit.prepare` / `submit`、独立字段提交、独立授权及 `oa:write:submit` scope；成功条件是恰好新增一条内部已发事项并可回读详情。真实会话 prepare 已通过，未点击发送、未发写请求，尚未做真实 submit |
| 请假申请草稿 W1 | 已实现并完成零写入真实预检 | 模板与 CAP4 字段已确认；一期仅开放无附件的 `年休`、`事假`、`调休`。prepare 已在真实会话通过，OA 计算天数/小时延迟到保存后核验；尚未做真实保存待发 |
| 宿主无关交互协议 | 已完成一期代码、契约与真实安全纵切 | 三类可信交互统一投影为 `agentbridge.interaction.v1`；CLI/MCP 支持按绑定用户查询和恢复，字段与授权重复恢复不重复产生计划或副作用；OpenClaw 2026.6.1 的 `url/webApp` presentation 结构已完成本机契约核对。2026-07-14 真实验证完成认证交互自动轮询与恢复，中心会话探测返回 118 个模板、`browserBridgeUsed=false`；未填写的 9 字段出差交互可查询且恢复返回 `INTERACTION_PENDING`，未打开 OA 表单或产生草稿 |
| 多用户安全主体 | 待真实验证 | 当前无第二个 OA 用户；代码级绑定与单用户身份核验已完成，OS/容器级双用户不可读性尚未证明 |
| 远程 MCP 部署 | 已完成内网 PoC，生产治理待补 | 已在固定内网 IP 上部署 HTTPS、内部 CA、Bearer 身份绑定和 OpenClaw 宿主适配；企业 OAuth/OIDC、正式 PKI/反向代理信任、限流、真实手机网络和第二用户仍待验证 |
| 更多中心写动作 | 已完成扩展一期 | 已实现出差草稿与正式提交、请假草稿、补签草稿与审批、新建会议；不同事项使用各自 Adapter 和回读规则，不对智能体暴露通用提交 API。新增正式提交与请假保存仍等待各自真实 commit 验收 |

首个 R0 纵切、可信卡片和内网远程 MCP 单用户门槛已经满足。下一组硬门槛是：两个不同真实用户在独立 Worker 安全主体中完成同一 R0 能力并证明 Profile、Cookie、下载和日志不能跨用户读取；同时为出差正式提交和请假草稿分别取得一次明确批准的真实 commit 回读证据。在此之前不宣称完成多用户隔离，也不把零写入预检表述为真实写入验收。
